  ...
  try {
    ...
    for (const item of cartItems) {
      ...
      const [rows] = await connection.query(
        `SELECT stock, nombre, supplier_id FROM ${tableToQuery} WHERE id = ? FOR UPDATE`,
        [item.id]
      );
...
// =========================
// Buscar órdenes por usuario (con filtros)
// =========================
export const findOrdersByUserId = async (userId, filters = {}) => {
  // Asegurar formato de nombres de mes en español si usas DATE_FORMAT en SQL
  await db.query("SET lc_time_names = 'es_ES'");

  let query = `SELECT o.id, o.total_amount, o.status, o.shipping_company, o.tracking_number, DATE_FORMAT(o.created_at, '%d de %M de %Y') as date, o.updated_at
               FROM orders o`;
  ...
  // Construimos la URL completa de la imagen en el backend usando BACKEND_URL (pasado como param)
  const backendUrl = (process.env.BACKEND_URL || '').replace(/\/$/, ''); // quitar slash final si hay
  ...
  const itemsQuery = `
    SELECT 
      ...
      IFNULL(
        (SELECT CONCAT(?, '/', image_url) FROM product_images WHERE product_id = p.id LIMIT 1),
        (SELECT CONCAT(?, '/', image_url) FROM insumo_images WHERE insumo_id = i.id LIMIT 1)
      ) as image
    FROM order_items oi
  ...
  // Pasamos backendUrl dos veces (para product_images y insumo_images) y luego el array de IDs
  const [items] = await db.query(itemsQuery, [backendUrl, backendUrl, orderIds]);

  // Mapear items a sus órdenes
...
// =========================
// Buscar orden por id (detalle)
 // firma: findOrderById(orderId, userId)
 // si userId es necesario para validar pertenencia; si no, pásalo como null o adapta
// =========================
export const findOrderById = async (orderId, userId) => {
  ...
  const [items] = await db.query(itemsQuery, [backendUrl, backendUrl, orderId]);

  return {
    id: orderDetails.id,
    total_amount: orderDetails.total_amount,
    status: orderDetails.status,
    shipping_address: orderDetails.shipping_address ? JSON.parse(orderDetails.shipping_address) : orderDetails.shipping_address || {},
...
// =========================
// Actualizar estado de orden (general)
// =========================
export const updateOrderStatus = async (orderId, updateData) => {
  const [result] = await db.query(
    `UPDATE orders SET status = ?, shipping_company = ?, tracking_number = ? WHERE id = ?`,
    [updateData.status, updateData.shipping_company, updateData.tracking_number, orderId]
  );
...
// =========================
// Actualizar status por supplier (con permiso)
// =========================
export const updateOrderStatusBySupplier = async (orderId, supplierId, updateData) => {
  ...
  try {
    ...
    const [items] = await connection.query(
      `SELECT oi.id FROM order_items oi LEFT JOIN products p ON oi.product_id = p.id LEFT JOIN insumos i ON oi.insumo_id = i.id WHERE oi.order_id = ? AND (p.supplier_id = ? OR i.supplier_id = ?)`,
...
// =========================
// Detalles de orden para supplier
// =========================
