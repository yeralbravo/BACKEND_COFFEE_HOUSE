...
const deleteImageFiles = (imageUrls) => {
  ...
  imageUrls.forEach(url => {
    try {
      // Obtener el nombre de archivo y construir la ruta dentro de uploads
      ...
      console.error('Error en deleteImageFiles:', err);
    }
  });
};
...
export const createInsumo = async (insumoData, images) => {
  ...
  try {
    ...
    // Si hubo imágenes, intentar limpiar los archivos que se hayan creado
    if (images && images.length > 0) {
...
export const updateInsumoById = async (insumoId, supplierId, insumoData, newImages) => {
  ...
  try {
    ...
    // Si vienen nuevas imágenes: borramos las antiguas (DB + FS) y guardamos las nuevas
    if (newImages && newImages.length > 0) {
      const [oldImagesRows] = await connection.query('SELECT image_url FROM insumo_images WHERE insumo_id = ?', [insumoId]);
      const oldImageUrls = oldImagesRows.map(r => r.image_url);

      await connection.query('DELETE FROM insumo_images WHERE insumo_id = ?', [insumoId]);

      // Borrar archivos físicos antiguos
      deleteImageFiles(oldImageUrls);

      // Insertar nuevas imágenes con URL completa
...
export const deleteInsumoById = async (insumoId, supplierId) => {
  ...
  try {
    ...
    // Seleccionamos las URLs de imagen asociadas al insumo si el insumo pertenece al supplier
